        ...
        for line in file:
            if not line.startswith('#') and line.strip():
                ...
                if parts[2] == 'gene':
                    ...
                    genes_info.append((chromosome, start_position, end_position, gene_name, gene_length, strand, f"{chromosome}:{expanded_start_position}..{expanded_end_position}"))
...
def main():
    ...
    genes_info = parse_gff(args.input_gff)

    with open(args.output_file, 'w') as out_file:
        for info in genes_info:
            out_file.write(f"{info[0]}\t{info[1]}\t{info[2]}\t{info[3]}\t{info[4]}\t{info[5]}\t{info[6]}\n")
